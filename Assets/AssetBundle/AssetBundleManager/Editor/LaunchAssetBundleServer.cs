...
namespace AssetBundles
{
    internal class LaunchAssetBundleServer : ScriptableSingleton<LaunchAssetBundleServer>
    {
        ...
        static void Run()
        {
            string pathToAssetServer = Path.GetFullPath("Assets/AssetBundle/AssetBundleManager/Editor/AssetBundleServer.exe");
        ...
        static string GetMonoProfileVersion()
        {
            ...
            for (int i = 0; i < foldersWithApi.Length; i++)
            {
                ...
                if (String.Compare(foldersWithApi[i], profileVersion, StringComparison.Ordinal) > 0)
                {
